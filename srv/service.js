...
module.exports = cds.service.impl(async function () {
    ...
    this.on('READ', 'SupplierTax', async (req) => {
        return await businessapi.run(req.query);
    });
    const {Label}=this.entities
        this.on('printForm', 'PurchaseOrderSrv', async (req) => {
            const {
                PurchaseOrderSrv, PurchaseOrderItem, PurOrdItemPricingElement,
                POSubcontractingComponent, PurchaseOrderScheduleLine, PurchaseOrderAccountAssignment,
                PurchaseOrderItemNote, PurchaseOrderNote, SupplierInfo
            } = this.entities;
            ...
            const SupplierTaxData = await businessapi.run(SELECT.from(this.entities.SupplierTax).where({ Supplier: rowData[0].Supplier }));
